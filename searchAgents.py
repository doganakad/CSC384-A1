            ...
            if not hitsWall:
                ...
                for corner in state[1]:
                    if corner != (nextx,nexty):
                        unvcorners.append(corner)
                successors.append(((nextpos, unvcorners), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    dist = []
    for node in unv:
        manhattanDist = util.manhattanDistance(pos,node)
        dist.append(manhattanDist)
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        ...
        for food2 in foodGrid.asList():
            d2 = util.manhattanDistance(food,food2)
            if d2 > left:
                left = d2
